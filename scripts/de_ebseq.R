...
Usage:
  de_ebseq.R  [--ngvector <PATH>] --level <TYPE> [--output-dir <PATH>] <count-mat-tsv> <n_rep1> <n_rep2>
...
norm_out_file <-
  paste0(gsub(".tsv", "", output_file), ".normalized_data_matrix.tsv")
...
n <- dim(DataMat)[2]
if (sum(num_reps) != n)
  stop("Total number of replicates given does not match the number of columns from the data matrix!")

conditions <-
  as.factor(rep(paste("C", 1:nc, sep = ""), times = num_reps))
Sizes <- MedianNorm(DataMat)
NormMat <- GetNormalizedMat(DataMat, Sizes)
ngvector <- NULL

if (ngvector_file != "#") {
  # CHANGE: Due to sort by feature_id, a single column to two columns
...
if (nc == 2) {
  ...
  results <-
    cbind(PP,
          fc_res$PostFC,
          fc_res$RealFC,
          unlist(EBOut$C1Mean)[rownames(PP)],
          unlist(EBOut$C2Mean)[rownames(PP)])
  colnames(results) <-
    c("PPEE", "PPDE", "PostFC", "RealFC", "C1Mean", "C2Mean")
